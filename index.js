...
window.cbk = function () {
    ...
    function onComplete(data) {
        ...
        addClickEventOption({ lnglat: data.position.getLng() + "," + data.position.getLat() });
    ...
    // 添加的地铁面板通过经纬度
    var addClickEventOption=function(lnglat){
        lnglat=lnglat || { lnglat: '116.539805,39.772915' };
        var StationId = mysubway.getNearStation(lnglat);
        addClickEventByStationId(StationId);
    }

    // 根据站点ID添加面板
    var addClickEventByStationId=function(StationId){
    ...
    mysubway.event.on("subway.complete", function () {
        ...
        mysubway.event.on("stationName.touch", function (argu, data) {
            addClickEventByStationId(data.id);
        });
        ...
        //地铁规划完成事件
        mysubway.event.on("subway.routeComplete", function (argu, res) {

            if(res.status===1 && res.data.buslist.length){
